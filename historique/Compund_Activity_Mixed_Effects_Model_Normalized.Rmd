---
title: "Compound Activity analysis with normalized data"
...
4.  **Control for Baseline Activity:** Treat baseline activity as a
    covariate in the model to control for individual differences in
    normal activity levels and assess how post-injection effects differ
    from baseline.
...
#### 2.2 Assess Baseline homogeneity

Assessing baseline homogeneity to help me decide if (1) I use baseline
as a co variate in my models (controling for individual diferences at
the start of the study) or (2) if I normalize the data to Baseline
(focus on relative changes and eventually reduce variability).

--> I decided to take it as covariate because in some cases normalization is not possible (some baseline values are 0 --> result INF)
and I dont want to exclude them in my analisis nor adding a small constant to the baseline values to avoid division by 0.
...
# Filter the data to include only baseline observations
baseline_data <- data_combined %>% filter(Is_Baseline == 1)

# List of variables to perform the Kruskal-Wallis test on
variables <- c(
...
# Store results in a list for later visualization
results <- list()

# Loop through each variable and perform the Kruskal-Wallis test
for (var in variables) {

  # Construct the formula for the Kruskal-Wallis test
  formula <- as.formula(paste(var, "~ Compound"))
  
  # Perform the test
  kruskal_test_result <- kruskal.test(formula, data = baseline_data)
  
  # Save results
  results[[var]] <- kruskal_test_result$p.value
  
  # Interpret the result
  if (kruskal_test_result$p.value > 0.05) {
    cat(var, ": baseline is homogenous across the different groups (p-value > 0.05).\n\n")
  } else {
    cat(var, ": baseline is NOT homogenous across the different groups (p-value <= 0.05).\n\n")
  }
}

# Reshape the data for easier plotting
baseline_long <- baseline_data %>%
  select(Compound, all_of(variables)) %>%
  pivot_longer(cols = all_of(variables), names_to = "Variable", values_to = "Value")

# Create a faceted boxplot
ggplot(baseline_long, aes(x = Compound, y = Value)) +
  geom_boxplot(aes(fill = Compound)) +
  facet_wrap(~ Variable, scales = "free_y", ncol = 3) +
  labs(title = "Baseline Homogeneity Across Variables",
       x = "Compound",
       y = "Value") +
  theme_minimal() +
...
# Performing normalization to baseline 

# Extract baseline values for each subject, compound, and dose combination
baseline_values <- data_combined %>%
  filter(Is_Baseline == 1) %>%
  select(ID, Compound, Dose..mg.kg., all_of(variables)) %>%
  rename_with(~ paste0("baseline_", .), all_of(variables))  # Rename baseline variables

# Join baseline values with the post-injection data to normalize
data_normalized <- data_combined %>%
  left_join(baseline_values, by = c("ID", "Compound", "Dose..mg.kg."))


# Normalize each variable by dividing the post-injection values by their respective baseline
for (var in variables) {
  baseline_var <- paste0("baseline_", var)  # Baseline variable name
  normalized_var <- paste0("normalized_", var)  # Normalized variable name
  data_normalized <- data_normalized %>%
    mutate(!!normalized_var := .data[[var]] / .data[[baseline_var]])
}

# Filter out baseline data (since normalization is only for post-injection)
data_normalized <- data_normalized %>% filter(Is_Baseline == 0)

# Summary of normalized data
summary_normalized <- data_normalized %>%
  group_by(Compound, Dose..mg.kg., Post.injection..h., ID) %>%
  summarise(across(starts_with("normalized_"), ~ round(mean(.x, na.rm = TRUE), 2)))

# Print the summary of normalized data
...
# When I included (1 | ID), the estimated variance for the random effect was extremely small (≈ 5.915e-08), suggesting that the random intercept for ID was not contributing meaningfully to the model, but I'm not sure if there is something else to do.reasonable to proceed without random effects. ?
# Should I fo fixed effects instead? (+ ID), that would mean that ID has a unique effect that should be estimated. 
# this is the comparison of both approaches: 
