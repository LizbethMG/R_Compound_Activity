...
plot_metric <- function(metric_name, y_label) {
  ...
  ggplot(data_combined, aes(x = as.factor(Post.injection..h.), y = .data[[metric_name]], fill = Compound_Dose)) +
    geom_boxplot(outlier.shape = NA, alpha = 0.6) +  # Suppress outliers to avoid double plotting
     geom_jitter(color = "#8b8c89", width = 0.2, alpha = 0.7) +   # Add jittered individual points
...
--\> I decided to take it as covariate because in some cases
normalization is not possible (baseline values are 0 --\> result INF)
and I dont want to exclude them in my analisis nor adding a small
constant to the baseline values to avoid division by 0.
